...
public class Type3Font extends BaseFont
{
     ...
     * @param chars an array of chars corresponding to the glyphs used (not used, present for compatibility
     * only)
     * @param colorized if <CODE>true</CODE> the font may specify color, if <CODE>false</CODE> no color
     * commands are allowed and only images as masks can be used
     */
    ...
    /**
     * Creates a Type3 font. This implementation assumes that the /FontMatrix is [0.001 0 0 0.001 0 0] or a
     * 1000-unit glyph coordinate system.
     * <p>
     * An example:
     * <p>
     * <
     * pre>
     * Document document = new Document(PageSize.A4); PdfWriter writer = PdfWriter.getInstance(document, new
     * FileOutputStream("type3.pdf")); document.open(); Type3Font t3 = new Type3Font(writer, false);
     * PdfContentByte g = t3.defineGlyph('a', 1000, 0, 0, 750, 750); g.rectangle(0, 0, 750, 750); g.fill(); g
     * = t3.defineGlyph('b', 1000, 0, 0, 750, 750); g.moveTo(0, 0); g.lineTo(375, 750); g.lineTo(750, 0);
     * g.fill(); Font f = new Font(t3, 12); document.add(new Paragraph("ababab", f)); document.close();
     * </pre>
     *
     * @param writer the writer
     * @param colorized if <CODE>true</CODE> the font may specify color, if <CODE>false</CODE> no color
