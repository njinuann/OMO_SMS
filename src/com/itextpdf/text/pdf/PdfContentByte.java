...
/**
 * <CODE>PdfContentByte</CODE> is an object containing the user positioned text and graphic contents of a
 * page. It knows how to apply the proper font encoding.
 */
public class PdfContentByte
{
     ...
     * <P>
     * <VAR>Flatness</VAR> sets the maximum permitted distance in device pixels between the mathematically
     * correct path and an approximation constructed from straight line segments.<BR>
     *
     ...
     * an <I>array</I> and a <I>phase</I>. The array specifies the length of the alternating dashes and gaps.
     * The phase specifies the distance into the dash pattern to start the dash.<BR>
     *
     ...
     * <P>
     * The <VAR>line join style</VAR> specifies the shape to be used at the corners of paths that are
     * stroked.<BR>
     * Allowed values are LINE_JOIN_MITER (Miter joins), LINE_JOIN_ROUND (Round joins) and LINE_JOIN_BEVEL
     * (Bevel joins).<BR>
     *
     * @param style a value
     */
     ...
     * style, it is possible for the miter to extend far beyond the thickness of the line stroking path. The
     * miter limit imposes a maximum on the ratio of the miter length to the line witdh. When the limit is
     * exceeded, the join is converted from a miter to a bevel.<BR>
     *
    ...
    /**
     * Modify the current clipping path by intersecting it with the current path, using the nonzero winding
     * number rule to determine which regions lie inside the clipping path.
     */
    public void clip()
    {
    ...
    /**
     * Modify the current clipping path by intersecting it with the current path, using the even-odd rule to
     ...
     * <P>
     * Sets the color space to <B>DeviceGray</B> (or the <B>DefaultGray</B> color space), and sets the gray
     * tint to use for filling paths.</P>
     *
     ...
     * <P>
     * Sets the color space to <B>DeviceGray</B> (or the <B>DefaultGray</B> color space), and sets the gray
     * tint to use for stroking paths.</P>
     *
     * @param gray a value between 0 (black) and 1 (white)
     */
     ...
     * <P>
     * Sets the color space to <B>DeviceRGB</B> (or the <B>DefaultRGB</B> color space), and sets the color to
     * use for filling paths.</P>
     * <P>
     ...
     * Changes the current color for filling paths (device dependent colors!).
     * <P>
     * Sets the color space to <B>DeviceCMYK</B> (or the <B>DefaultCMYK</B> color space), and sets the color
     * to use for filling paths.</P>
     * <P>
    ...
    /**
     * Appends a straight line segment from the current point <I>(x, y)</I>. The new current point is <I>(x,
     * y)</I>.
     *
     * @param x new x-coordinate
     * @param y new y-coordinate
     ...
     * {@link com.itextpdf.text.Rectangle#isUseVariableBorders() Rectangle.isUseVariableBorders} = true.
     *
    ...
    /**
     * Closes the current subpath by appending a straight line segment from the current point to the starting
    ...
    /**
     * Closes the path, fills it using the non-zero winding number rule to determine the region to fill and
     * strokes it.
     */
    ...
    /**
     * Adds an <CODE>Image</CODE> to the page. The <CODE>Image</CODE> must have absolute positioning. The
     * image can be placed inline.
     *
     * @param image the <CODE>Image</CODE> object
    ...
    /**
     * Adds an <CODE>Image</CODE> to the page. The positioning of the <CODE>Image</CODE> is done with the
     * transformation matrix. To position an <CODE>image</CODE> at (x,y) use addImage(image, image_width, 0,
     * 0, image_height, x, y).
     *
     * @param image the <CODE>Image</CODE> object
     ...
     * Adds an <CODE>Image</CODE> to the page. The positioning of the <CODE>Image</CODE> is done with the
     * transformation matrix. To position an <CODE>image</CODE> at (x,y) use addImage(image, image_width, 0,
     * 0, image_height, x, y). The image can be placed inline.
     *
     * @param image the <CODE>Image</CODE> object
     * @param a an element of the transformation matrix
     * @param b an element of the transformation matrix
    ...
    /**
     * Saves the graphic state. <CODE>saveState</CODE> and <CODE>restoreState</CODE> must be balanced.
     */
    ...
    /**
     * Restores the graphic state. <CODE>saveState</CODE> and <CODE>restoreState</CODE> must be balanced.
     */
     ...
     * <P>
     * The leading parameter is measured in text space units. It specifies the vertical distance between the
     * baselines of adjacent lines of text.</P>
     *
    ...
    /**
     * A helper to insert into the content stream the <CODE>text</CODE> converted to bytes according to the
     * font's encoding.
     *
    ...
    /**
     * Computes the width of the given string taking in account the current values of "Character spacing",
     * "Word Spacing" and "Horizontal Scaling". The additional spacing is not computed for the last character
     * of the string.
     *
     ...
     * <P>
     * (x1, y1) and (x2, y2) are the corners of the enclosing rectangle. Angles, measured in degrees, start
     * with 0 to the right (the positive X axis) and increase counter-clockwise. The arc extends from startAng
     * to startAng+extent. I.e. startAng=0 and extent=180 yields an openside-down semi-circle.
     * <P>
     * The resulting coordinates are of the form float[]{x1,y1,x2,y2,x3,y3, x4,y4} such that the curve goes
     * from (x1, y1) to (x4, y4) with (x2, y2) and (x3, y3) as their respective Bezier control points.
     * <P>
     * Note: this code was taken from ReportLab (www.reportlab.org), an excellent PDF generator for Python
     * (BSD license: http://www.reportlab.org/devfaq.html#1.3 ).
     *
    ...
    /**
     * Draws a partial ellipse inscribed within the rectangle x1,y1,x2,y2, starting at startAng degrees and
     * covering extent degrees. Angles start with 0 to the right (+x) and increase counter-clockwise.
     *
     * @param x1 a corner of the enclosing rectangle
     * @param y1 a corner of the enclosing rectangle
     * @param x2 a corner of the enclosing rectangle
     ...
     * @param xstep the desired horizontal spacing between pattern cells. May be either positive or negative,
     * but not zero.
     * @param ystep the desired vertical spacing between pattern cells. May be either positive or negative,
     * but not zero.
    ...
    /**
     * Create a new colored tiling pattern. Variables xstep and ystep are set to the same values of width and
     * height.
     *
     * @param width the width of the pattern
     * @param height the height of the pattern
    ...
    /**
     * Create a new uncolored tiling pattern. Variables xstep and ystep are set to the same values of width
     * and height.
     *
     * @param width the width of the pattern
     * @param height the height of the pattern
     ...
     * <CODE>PdfContentByte</CODE> or in another template. Templates are only written to the output when the
     * document is closed permitting things like showing text in the first page that is only defined in the
     * last page.
     *
     ...
     * to use for filling paths.</P>
     * <P>
     * This method is described in the 'Portable Document Format Reference Manual version 1.3' section 8.5.2.1
     * (page 331).</P>
     * <P>
     * Following the PDF manual, each operand must be a number between 0 (no ink) and 1 (maximum ink). This
     * method however accepts only integers between 0x00 and 0xFF.</P>
     *
     ...
     * <P>
     * Following the PDF manual, each operand must be a number between 0 (minimum intensity) and 1 (maximum
     * intensity). This method however accepts only integers between 0x00 and 0xFF.</P>
     *
    ...
    /**
     * Implements a link to other part of the document. The jump will be made to a local destination with the
     * same name, that must exist.
     *
     ...
     * @return <CODE>true</CODE> if the local destination was added, <CODE>false</CODE> if a local destination
    ...
    /**
     * Gets a duplicate of this <CODE>PdfContentByte</CODE>. All the members are copied by reference but the
     * buffer stays different.
     *
    ...
    /**
     * Gets a <CODE>Graphics2D</CODE> to write on. The graphics are translated to PDF commands as shapes. No
     * PDF fonts will appear.
     *
    ...
    /**
     * Gets a <CODE>Graphics2D</CODE> to print on. The graphics are translated to PDF commands as shapes. No
     * PDF fonts will appear.
     *
     * @param width the width of the panel
     * @param height the height of the panel
    ...
    /**
     * Begins a graphic block whose visibility is controlled by the <CODE>layer</CODE>. Blocks can be nested.
     * Each block must be terminated by an {@link #endLayer()}
     * .<p>
     * Note that nested layers with {@link PdfLayer#addChild(PdfLayer)} only require a single call to this
     * method and a single call to {@link #endLayer()}; all the nesting control is built in.
     *
     ...
     * @param obj the colorspace. A <CODE>null</CODE> or <CODE>PdfNull</CODE> removes any colorspace with the
     * same name
     */
     ...
     * @param inline <CODE>true</CODE> to include the property in the content or <CODE>false</CODE> to include
     * the property in the resource dictionary with the possibility of reusing
     */
    ...
    /**
     * Checks for any dangling state: Mismatched save/restore state, begin/end text, begin/end layer, or
     * begin/end marked content sequence. If found, this function will throw. This function is called
     * automatically during a reset() (from Document.newPage() for example), and before writing itself out in
     * toPdf(). One possible cause: not calling myPdfGraphics2D.dispose() will leave dangling saveState()
     * calls.
     *
     * @since 2.1.6
